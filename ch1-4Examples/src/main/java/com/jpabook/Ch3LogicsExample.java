...
public class Ch3LogicsExample {
    ...
    public void entityLifeCycle() {
        ...
        try {
            //비영속 상태
            Member member = new Member("HelloJPA");
...
//            em.remove(member);  //영구 저장을 데이터베이스에서 지우겟어..??
     ...
     */
    public void notInFistCache() {
        long memberId = addMember();//member commit
     ...
     * 영속 엔티티 동일성 보장 예제
     */
    public void entityIdentity() {

        long memberId = addMember();//add member
     ...
     * 트랜잭션을 지원하는 쓰기 지연
     */
    public void transactionWriteBehind() {
    ...
    public void merge() {
        ...
        Member member = createMemberForMerge("member");
    ...
    private Member createMemberForMerge(String userName) {
    ...
    private long addMember() {
        ...
        try {
            ...
            tx.rollback();
            throw new RuntimeException("Failed Add new member");
        } finally {
            em.close();
        }
    ...
    private Member findMember(long memberId) {
        ...
        try {
            ...
            tx.rollback();
            throw new RuntimeException("Failed Find Member : memberId(" + memberId + ")");
        } finally {
            em.close();
        }
    }
